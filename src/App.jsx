...
function BrowserPreview() {
  ...
  const tabs = open.map(i => <button key={i} className={`preview-tab ${selected === i ? 'active' : ''}`} onClick={() => setSelected(i)} aria-pressed={selected === i}><span className="preview-favicon">{pages[i].icon}</span><span>{pages[i].name}</span><span className="preview-tab-close" onClick={e => { e.stopPropagation(); if (open.length > 1) { const rest = open.filter(n => n !== i); setOpen(rest); if (selected === i) setSelected(rest[0]); } }} aria-hidden="true"><Icon name="close" size={12} /></span></button>);
  const addTab = () => { const next = [0, 1, 2].find(i => !open.includes(i)); if (next !== undefined) { setOpen([...open, next]); setSelected(next); } else setSelected((selected + 1) % 3); };
...
export default function App() {
  ...
  return <>
    ...
    <main id="top"><section className="hero content"><div className="eyebrow"><span className="square"/> QUANTUM FOR MACOS</div><h1>Less browser.<br/><em>More web.</em></h1><p>A lightweight macOS browser that uses the WebKit already on your Mac.</p><div className="hero-actions"><a className="button primary" href="#download"><Apple/> Get Quantum <Icon name="arrow" size={15}/></a><a className="text-button" href="#preview">Explore <Icon name="arrow" size={15}/></a></div></section>
      <section className="showcase" id="preview"><div className="content"><div className="section-top"><div className="eyebrow"><span className="square"/> THE BROWSER</div><h2>Just the web.</h2></div><BrowserPreview/></div></section>
      <section className="features" id="features"><div className="content"><div className="section-top"><div className="eyebrow"><span className="square"/> ESSENTIALS</div><h2>Less, but better.</h2></div><div className="feature-grid">{features.map((item, i) => <article className="feature-card" key={i}><div className="feature-icon">{item.icon}</div><h3>{item.title}</h3></article>)}</div></div></section>
      <section className="faq" id="faq"><div className="content"><div className="section-top"><div className="eyebrow"><span className="square"/> FAQ</div><h2>Simple answers.</h2></div><div className="faq-items">{faq.map(([q,a],i)=><div className="faq-item" key={q}><button aria-expanded={expanded === i} aria-controls={`faq-${i}`} onClick={() => setExpanded(expanded === i ? null : i)}>{q}<span className={expanded === i ? 'turned' : ''}><Icon name="chevron" size={19}/></span></button>{expanded === i && <p id={`faq-${i}`}>{a}</p>}</div>)}</div></div></section>
      <section className="download" id="download"><div className="content"><div className="eyebrow"><span className="square"/> MACOS</div><h2>Get Quantum.</h2><div className="download-option"><Apple size={20}/><span>macOS</span><span className="download-tag">DMG</span>{DMG ? <a className="button secondary" href={DMG} rel="noopener noreferrer"><Icon name="download" size={16}/> Download</a> : <span className="coming">Coming soon</span>}</div></div></section>
